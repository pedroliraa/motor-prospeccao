...
interface Props {
  etapaAtual: number; // 0-3, -1 = inativo
  total: number;
  encontradas: number;
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between text-xs text-gray-500">
        <span>{etapaAtual < 0 ? 'Aguardando execução' : ETAPAS[etapaAtual]?.label + '...'}</span>
        <span>{pct}%</span>
      </div>
        ...
        {ETAPAS.map((e, i) => {
          ...
          return (
            <div key={e.key} className="flex items-center gap-1.5">
              ...
              <span className={`text-xs ${active ? 'font-medium text-gray-800' : 'text-gray-400'}`}>
                {e.label}
              </span>
            </div>
          );
